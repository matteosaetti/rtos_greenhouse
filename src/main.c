...
int main() {
    ...
    gpio_init(VALVE1_PIN); gpio_set_dir(VALVE1_PIN, GPIO_OUT); gpio_put(VALVE1_PIN, 0);
    gpio_init(VALVE2_PIN); gpio_set_dir(VALVE2_PIN, GPIO_OUT); gpio_put(VALVE2_PIN, 0);
    gpio_init(VALVE3_PIN); gpio_set_dir(VALVE3_PIN, GPIO_OUT); gpio_put(VALVE3_PIN, 0);
    gpio_init(RETURN_PIN); gpio_set_dir(RETURN_PIN, GPIO_OUT); gpio_put(RETURN_PIN, 1);


    qSensor = xQueueCreate(QUEUE_SENSOR_LEN, sizeof(sensor_msg_t));
    for(int i=0;i<3;i++) qValveCmd[i] = xQueueCreate(QUEUE_CMD_LEN, sizeof(valve_cmd_t));
    ...
    for(int i=0;i<3;i++){ sysState.valveOn[i] = false; sysState.humidity[i]=0; }
